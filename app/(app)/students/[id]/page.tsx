...
import { useStudent, useUpdateStudent, useSchools } from "@/lib/hooks/queries";
...
export default function EditStudentPage() {
  ...
  useEffect(() => {
    ...
    setSchoolId(student.school_id ?? student.school?.id ?? null);
    setGrade(student.grade ?? "");
    setGender(student.gender ?? "male");
    setSizes({
      shirt: student.sizes?.shirt ?? "",
      pants: student.sizes?.pants ?? "",
      skirt: student.sizes?.skirt ?? "",
      shoes: student.sizes?.shoes ?? "",
      jacket: student.sizes?.jacket ?? "",
    });
    if (student.school) {
      setSchoolSearch(getLocalizedName(student.school.name, locale));
    }
  }, [student, locale]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      toast(locale === "ar" ? "تم تحديث بيانات الطالب" : "Student updated", "success");
      router.push("/account");
    } catch (err) {
  ...
  if (!student) {
    return (
      <div className="text-center py-20 text-gray-400">
        <p>{locale === "ar" ? "الطالب غير موجود" : "Student not found"}</p>
      </div>
    );
  }
    ...
    <div className="max-w-lg mx-auto px-4 py-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div>
          ...
          <div className="relative mb-2">
            ...
            <input
              type="text"
              value={schoolSearch}
              onChange={(e) => { setSchoolSearch(e.target.value); setSchoolId(null); }}
        ...
        <div className="flex gap-3 pt-2">
          ...
          <button type="submit" disabled={updateMut.isPending} className="flex-1 bg-[#318B9B] text-white font-bold py-3.5 rounded-2xl text-sm hover:bg-[#1a5f6b] disabled:opacity-60 transition-colors active:scale-95">
            {updateMut.isPending ? "..." : (locale === "ar" ? "حفظ التغييرات" : "Save Changes")}
          </button>
        </div>
      </form>
    </div>
